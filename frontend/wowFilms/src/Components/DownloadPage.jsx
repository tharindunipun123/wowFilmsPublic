...
const DownloadPage = () => {
  ...
  const handleDownload = () => {
    if (film && film.filmUrl) {
      window.location.href = `http://109.199.99.84:3000/download/${id}`; // Updated to use the new download endpoint
    }
  };
  ...
  return (
    <div className="download-page-wrapper">
      ...
      <img src={logo} alt="Logo" className="logo" /> {/* New Logo Image */}
        ...
        <img src={`http://109.199.99.84:3000/${film.thumbnailUrl}`} alt={film.name} className="thumbnail" />
        <h1>{film.name}</h1>
